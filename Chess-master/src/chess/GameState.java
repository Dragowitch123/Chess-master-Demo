...
final class GameState {
     ...
     * @throws IllegalStateException if there is no allied king or more than one
     *                               allied king
     */
    ...
    /**
     * Creates marks on the board letting the user know where the piece can be
     * moved.
     */
    private void letUserKnowLegalMoves() {
        ...
        if (enPassant != null) {
            final boolean canCaptureEnPassant = from.y == enPassant.y + 1 && Math.abs(from.x - enPassant.x) == 1;
    ...
    /**
     * Moves the piece. If it is a pawn moving into a promotion square, ask the user
     * what to promote the pawn to and promote the pawn based on user input.
     *
    ...
    /**
     * Performs en passant. En passant is a move which lets a pawn capture a pawn
     * which just moved two squares as if it only moved one square immediately after
     * it happens.
     */
    ...
    /**
     * Draw if insufficient mating material. Mating material is any pieces which
     * could force a checkmate.
     */
     ...
     * @return is lone King against: lone King, or King and Knight, or King and
     *         Bishop, or King and two Knights
     */
    ...
    private boolean canQueenSideCastle() {
        ...
        final boolean isClearPath = hasMoved(Chess.getBoard(new Point(0, 7))) && Chess.getBoard(new Point(1, 7)) == null
                && Chess.getBoard(new Point(2, 7)) == null && Chess.getBoard(new Point(3, 7)) == null
                && hasMoved(Chess.getBoard(new Point(4, 7)));
        final boolean isNotPassingThroughCheck = !king.isCheck(new Point(4, 7)) && !king.isCheck(new Point(3, 7))
                && !king.isCheck(new Point(2, 7));
    ...
    private boolean canKingSideCastle() {
        ...
        final boolean isNotPassingThroughCheck = !king.isCheck(new Point(4, 7)) && !king.isCheck(new Point(5, 7))
                && !king.isCheck(new Point(6, 7));
        return isLockedOnKing && isClearPath && isNotPassingThroughCheck;
    }
    ...
    /**
     * Moves the piece without setting the piece to moved state. Used when checking
     * the board and not actually moving pieces on it.
     *
     * @param me    the piece to move
     * @param start the location to move from
     * @param end   the location to move to
